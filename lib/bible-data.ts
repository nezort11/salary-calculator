...
export const FIXED_PLANS: FixedPlan[] = [
  {
    id: "old-testament-fixed-two-years",
    name: "Ветхий Завет за 2 года (фиксированный план)",
    description:
      "Фиксированный план чтения Ветхого Завета за 2 года с ежедневными чтениями.",
    testament: "old",
    readings: [
      "Иов.29",
      "Иов.30",
...
export function formatReadingPlanContinuous(
  ...
  return readingPlan
    .map((day) => {
      ...
      const month = format(day.date, "LLLL", { locale });
      const readings = day.readings.join("; ");
      // Underline only the date part using HTML underline tag for proper PDF rendering
      const datePart = `${dayOfWeek}, ${dayOfMonth} ${month}`;
      return `<u>${datePart}</u> — ${readings}`;
    })
    .join("; ");
}
